...
# --- 천체별 가상/예시 데이터베이스 (헤더에 없을 때 매칭용) ---
# 실제 천문학적 데이터를 기반으로 한 예시 값들일세.
OBJECT_DB = {
    "M31": {"name": "안드로메다 은하 (M31)", "temp": "약 4,000 ~ 10,000 K (은하 내 별들의 평균)", "dist": "약 250만 광년"},
    "M42": {"name": "오리온 성운 (M42)", "temp": "약 10,000 K (전리수소영역 가스 온도)", "dist": "약 1,344 광년"},
    "M45": {"name": "플레이아데스 성단 (M45)", "temp": "약 30,000 K (주계열 B형 별 중심)", "dist": "약 444 광년"},
    "MOON": {"name": "달 (Moon)", "temp": "약 120℃ (낮) / -130℃ (밤)", "dist": "약 38만 4,400 km"},
    "JUPITER": {"name": "목성 (Jupiter)", "temp": "약 -108℃ (대기 상층부)", "dist": "약 4~6 AU (태양-지구 거리의 4~6배)"}
}
        ...
        with fits.open(uploaded_file) as hdul:
            ...
            if image_hdu is None:
                ...
                with col1:
                    ...
                    obj_name = "Unknown"
                    if 'OBJECT' in header:
                        obj_name = str(header['OBJECT']).upper().strip()
                        st.text(f"관측 대상: {header['OBJECT']}")
                    if 'EXPTIME' in header:
                        st.text(f"노출 시간: {header['EXPTIME']} 초")

                    st.header("물리량 및 천체 특성")
                    mean_brightness = np.mean(data)
                    st.metric(label="이미지 전체 평균 밝기", value=f"{mean_brightness:.2f}")

                    # --- 🌟 [추가 변수] 온도 및 거리 계산/가져오기 로직 🌟 ---
                    # 1. 먼저 FITS 헤더에 관련 키워드가 있는지 확인 (예: 'TEMP', 'DIST' 등 사용자 정의 헤더가 있을 경우)
                    obj_temp = header.get('TEMP', None)
                    obj_dist = header.get('DIST', None)

                    # 2. 헤더에 없다면 위에서 정의한 데이터베이스(OBJECT_DB)에서 천체 이름을 검색해 매칭
                    if obj_temp is None or obj_dist is None:
                        # 관측 대상 이름이 DB 키에 포함되어 있는지 확인 (예: 'M42' 혹은 'ORION')
                        matched = False
                        for key, info in OBJECT_DB.items():
                            if key in obj_name:
                                obj_temp = obj_temp or info["temp"]
                                obj_dist = obj_dist or info["dist"]
                                matched = True
                                break
                        
                        if not matched:
                            obj_temp = obj_temp or "헤더 정보 없음 (분광 데이터 필요)"
                            obj_dist = obj_dist or "헤더 정보 없음 (시차/거리 측정 필요)"

                    # 3. Streamlit 화면에 메트릭(Metric) 형태로 출력
                    st.metric(label="🌡️ 천체의 예상 온도", value=str(obj_temp))
                    st.metric(label="📏 지구와의 거리", value=str(obj_dist))
